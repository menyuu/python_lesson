import sqlite3

from flask import Flask
from flask import g
from flask import render_template
from flask import request
from flask import Response


app = Flask(__name__)

# データベースへのアクセス
# flask のドキュメントに記載されている呼び出し方
def get_db():
    db = getattr(g, '_database', None)
    if db is None:
        db = g._database = sqlite3.connect('test_sqlite.db')
    return db

# データベースの close
# 引数に exception を渡す
@app.teardown_appcontext
def close_connection(exception):
    db = getattr(g, '_database', None)
    if db is not None:
        db.close()

@app.route('/employee', methods=['POST', 'PUT', 'DELETE'])
@app.route('/employee/<name>', methods=['GET'])
def employee(name=None):
    # データベースのテーブルはアプリケーションの初めに作るのが好ましい
    db = get_db()
    curs = db.cursor()
    curs.execute(
        'CREATE TABLE IF NOT EXISTS persons('
        'id INTEGER PRIMARY KEY AUTOINCREMENT, name STRING)'
    )
    db.commit()

    # GET での取り出し
    name = request.values.get('name', name)
    if request.method == 'GET':
        curs.execute('SELECT * FROM persons WHERE name = "{}"'.format(name))
        # あれば fetchone で一番はじめめに持ってくる
        person = curs.fetchone()
        if not person:
            return  'No', 404
        user_id, name = person
        return '{}:{}'.format(user_id,name), 200

    # POST でのデータの作成
    if request.method == 'POST':
        curs.execute(
            'INSERT INTO persons(name) values("{}")'.format(name)
        )
        db.commit()
        return 'created {}'.format(name), 201

    # PUT でのデータの更新
    if request.method == 'PUT':
        new_name = request.values['new_name']
        curs.execute(
            'UPDATE persons set name = "{}" WHERE name = "{}"'.format(new_name, name)
        )
        db.commit()
        return 'updated {}: {}'.format(name, new_name), 200

    if request.method == 'DELETE':
        curs.execute(
            'DELETE from persons WHERE name = "{}"'.format(name)
        )
        db.commit()
        return 'deleted {}'.format(name), 200

    curs.close()

# GET
# URL の指定
@app.route('/')
def hello_world():
    return 'top'

# @app.route('/hello')
# def hello_world2():
#     return 'hello world'

# URL に <> で指定することで、引数として渡すことができる
# 指定しなければ Not_Found で返ってくるのでもう一つデコレーターを作る
@app.route('/hello')
@app.route('/hello/<username>')
# デフォルト引数に None を渡す
def hello_world2(username=None):
    # return 'hello world! {}'.format(username)
    # テンプレートファイル(HTML)などを使って記述することができる
    return render_template('hello.html', username=username)

# POST, PUT, DELETE
@app.route('/post', methods=['POST', 'PUT','DELETE'])
def show_post():
    return str(request.values['username'])

def main():
    app.debug = True
    app.run()
    # デフォルトでは app.run(host='127.0.0.1, port=5000) となっている
    # app.run(host='127.0.0.1', port=5000)

if __name__ == '__main__':
    main()